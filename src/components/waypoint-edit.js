...
const createTypeOptionMarkup = (type) => {
  return `<div class="event__type-item">
            ...
            <label class="event__type-label  event__type-label--${type.value}" for="event-type-${type.value}-1">${capitalizeFirstLetter(type.value)}</label>
          </div>`;
};
...
const createTypeListMarkup = (types, group) => {
  ...
  return `<fieldset class="event__type-group">
            <legend class="visually-hidden">${capitalizeFirstLetter(group)}</legend>
...
const createSightMarkup = (sight) => {
  return `<img class="event__photo" src="${sight}" alt="Event photo">`;
};

const createSightListMarkup = (sights) => {
  let sightsMarkup = ``;

  sights.forEach((sight) => {
    sightsMarkup += createSightMarkup(sight);
  });
...
export const createWaypointEditMarkup = (waypoint, isNew) => {
  const {type, city, time, price, offers, destination, sights} = waypoint;
